...
from rnn_model import GRUDecoder, DayBiGRUEncoder, DayBiGRUEncoderForPhonemes, GRUEncoder, WhisperGRUEncoderForPhonemes

class PretrainingPhoneme_Trainer:
    """
    ...
    def __init__(self, args):
        ...
        self.encoder = GRUEncoder(
            ...
            n_classes=41,  # unused in whisper-encoder mode, but keep for config compatibility
            ...
            head_num_blocks=int(self.args["model"].get("head_num_blocks", 0)),
            head_norm=self.args["model"].get("head_norm", "none"),
            head_dropout=float(self.args["model"].get("head_dropout", 0.0)),
            head_activation=self.args["model"].get("head_activation", "gelu"),
            input_speckle_p=float(self.args["model"].get("input_speckle_p", 0.0)),
            input_speckle_mode=self.args["model"].get("input_speckle_mode", "feature"),
            d_model=d_model,
        ).to(self.device)
        
        self.model = WhisperGRUEncoderForPhonemes(
